...
@login_required
def train(projid):
    project_name = classes.User_Project.query.filter_by(project_id=projid) \
        .first().project_name
    proj = classes.Project.query.filter_by(project_id=projid).first()
    last_train_asp_ratio = proj.last_train_asp_ratio
    ...
    if form.validate_on_submit():
        ...
        for f in files:
            filename = secure_filename(f.filename)
            file_content = f.stream.read()
            s3_filepath = f"{projid}/predict/{filename}"

            k = Key(bucket)
            k.key = s3_filepath
            k.set_contents_from_string(file_content)
            filepaths.append(s3_filepath)

            # get aspect ratio somewhere here

        # call Miguel's train function 
        model = train(last_train_asp_ratio, filepaths)
        k_m = Key(bucket)
        k_m.key = f'/{projid}/model/'  # how to name the model
        # k_m.set_contents_from_string('')    # save the model

    return render_template('train.html', projnm=project_name, form=form)
