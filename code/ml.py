...
def im_write(im, client, s3_path, bucket_name=BUCKET_RESIZE, tmp_p='resize'+TMP_IMG_FILE):
    cv2.imwrite(tmp_p, im)
    client.upload_file(tmp_p, bucket_name, Key=s3_path, ExtraArgs={
                       'ACL': 'bucket-owner-full-control'})
    os.remove(tmp_p)


def resize_images(df, aspect_r, client, project_name, org_bucket=BUCKET_ORIG, dest_bucket=BUCKET_RESIZE):
...
class ProjectDS(Dataset):
    ...
    def __init__(self, df, lbl2idx, client, project_name, bucket_name=BUCKET_RESIZE):
...
#     @property
#     def idx2lbl(self): return self.dataset.idx2lbl
...
def save_model(m, client, project_name, bucket_name=BUCKET_ORIG, tmp_p=TMP_MODEL_FILE, s3_p=None):
...
def load_model(m, client, project_name, bucket_name=BUCKET_ORIG, tmp_p=TMP_MODEL_FILE):
...
class DenseNet(nn.Module):
    ...
    def __init__(self, out_size: int = 1, pretrained: bool = True, freeze: str = False):
        '''
...
class OptimizerWrapper:
    ...
    def __init__(self, policy, model, n_epochs, dl, max_lr, wd=0, alpha=1. / 3):
        ...
        self.optimizer = optim.Adam(
            [{'params': p, 'lr': lr, 'mom': (mom, .999)}
             for p, lr, mom in zip(param_groups, diff_range(lr_0, alpha), diff_range(mom_0, 1))]
        )
...
def train_model(n_epochs, model, train_dl, n_lbls, best_loss, valid_dl=None, max_lr=.01, wd=0, alpha=1. / 3, project_name=None,
                save_path=None, unfreeze_during_loop: tuple = None, client=CLIENT):
    model.train()
    if n_lbls > 2:
    ...
    for epoch in range(n_epochs):
        ...
        print(
            f'Ep. {epoch+1} - train loss {agg_loss/agg_div:.4f} -  val loss {val_loss:.4f} AVG F1 {measure:.4f}')
...
def send_email(receiver_name, receiver_email):
    ...
    You training has finished. You can log-in and start using your brand new model.
